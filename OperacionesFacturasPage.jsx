  ...
  return (
    <div className="fade-in" style={S.page}>
      ...
      <style>{`
        ...
        .qf-facturas-table tbody tr {
          border-bottom: 1px solid var(--qf-border);
        }
        .qf-facturas-table tbody tr:hover {
          background: #f8fafc;
        }
        .qf-facturas-table td {
          border-bottom: 1px solid var(--qf-border);
        }
      `}</style>
      ...
      <div style={S.actionBar}>
        <div style={S.toolbarLeft}>
          ...
          <span style={S.toolbarChip}>ERP Operaciones</span>
          <span style={S.toolbarChip}>Facturas</span>
        </div>
        <div style={S.toolbarRight}>
          <span style={S.toolbarMetric}>Obs: <b>{metrics.observadas}</b></span>
          <span style={S.toolbarMetric}>Fondos: <b>{metrics.fondos}</b></span>
        </div>
      </div>
        ...
        <div style={S.stickyTools}>
          <div style={S.cardTitleWrap}>
            <div>
              ...
              <div style={S.cardSubtitle}>Búsqueda, paginación y acciones principales</div>
            </div>
            <button className="btn btn-primary btn-sm" onClick={() => setModal({ type: 'nuevo' })} style={S.newButton}>
              <span style={{ color: '#4CAF50', fontWeight: 800, fontSize: 16 }}>+</span> Nueva Factura
            </button>
          </div>

          <div style={S.controlRow}>
            <select className="filter-input" value={pageSize} onChange={e => { setPageSize(Number(e.target.value)); setPage(1) }} style={S.pageSizeSelect}>
            ...
            <div style={S.topPagination}>
              <button className="btn btn-secondary btn-sm" style={S.pageNavBtn} disabled={page <= 1 || loading} onClick={() => setPage(1)}>«</button>
              <button className="btn btn-secondary btn-sm" style={S.pageNavBtn} disabled={page <= 1 || loading} onClick={() => setPage(p => Math.max(1, p - 1))}>‹</button>
              <span style={S.pageMini}>Pág. {page} de {totalPages}</span>
              <button className="btn btn-secondary btn-sm" style={S.pageNavBtn} disabled={page >= totalPages || loading} onClick={() => setPage(p => Math.min(totalPages, p + 1))}>›</button>
              <button className="btn btn-secondary btn-sm" style={S.pageNavBtn} disabled={page >= totalPages || loading} onClick={() => setPage(totalPages)}>»</button>
            </div>
            ...
            <div style={S.searchWrap}>
              ...
              <input className="filter-input" placeholder={campo === 'all' ? 'Búsqueda global...' : `Buscar por ${camposBusqueda.find(f => f.value === campo)?.label || ''}...`} value={busqueda} onChange={e => setBusqueda(e.target.value)} style={S.searchInput} />
            </div>

            <button className="btn btn-secondary btn-sm" onClick={limpiar} style={S.resetInlineBtn}>Limpiar</button>
            {loading && <span style={S.loadMini}>...</span>}
          </div>

          <div style={S.smartTotals}>
            <span><b>{facturas.length}</b> mostradas</span>
            <span><b>{money(metrics.totalNeto)}</b> neto</span>
            <span><b>{money(metrics.totalMonto)}</b> monto</span>
            <span><b>{metrics.registradas}</b> registradas</span>
            <span><b>{metrics.observadas}</b> observadas</span>
          </div>
        </div>

        <div style={{ overflow: 'auto', width: '100%', maxHeight: compactMode ? 'calc(100vh - 330px)' : 'calc(100vh - 390px)', borderTop: '1px solid var(--qf-border)' }}>
          {loading && facturas.length === 0 ? <div style={{ padding: 40, textAlign: 'center' }}><span className="spinner dark" /></div> : facturas.length === 0 ? <div className="empty-state"><div className="icon">🧾</div><p>No se encontraron facturas</p></div> : (
            <table className="qf-table qf-facturas-table" style={{ width: '100%', minWidth: 1280, tableLayout: 'auto', fontSize: compactMode ? 10.5 : 12 }}>
              <thead><tr>
...
const S = {
  ...
  actionBar: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 8, background: '#fff', border: '1px solid var(--qf-border)', borderRadius: 12, padding: '6px 10px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' },
  ...
  toolbarChip: { fontSize: 10, fontWeight: 800, color: 'var(--qf-navy)', background: '#e8eef5', borderRadius: 999, padding: '3px 8px', border: '1px solid #c9d7e6' },
  toolbarMetric: { fontSize: 10, color: 'var(--qf-text-light)', background: '#f8fafc', border: '1px solid var(--qf-border)', borderRadius: 999, padding: '3px 8px' },
  ...
  cardTitleWrap: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '8px 10px 6px' },
  cardTitle: { margin: 0, fontSize: 16, fontFamily: 'Montserrat', color: 'var(--qf-navy)' },
  cardSubtitle: { marginTop: 1, fontSize: 10, color: 'var(--qf-text-light)', fontWeight: 600 },
  newButton: { display: 'flex', alignItems: 'center', gap: 4, height: 26, borderRadius: 8, flexShrink: 0 },
  ...
  fieldSelect: { width: 'auto', minWidth: 135, height: 24, fontSize: 10, padding: '0 22px 0 8px' },
  searchInput: { width: '100%', minWidth: 180, maxWidth: 340, height: 24, fontSize: 10, paddingLeft: 30 },
  searchWrap: { position: 'relative', width: 260, maxWidth: '32vw', flex: '1 1 220px' },
  searchIcon: { position: 'absolute', left: 9, top: '50%', transform: 'translateY(-50%)', fontSize: 13, color: '#8a9bb5', pointerEvents: 'none', zIndex: 1 },
  controlRow: { display: 'flex', gap: 5, alignItems: 'center', flexWrap: 'nowrap', padding: '3px 10px', background: '#f8fafc', borderTop: '1px solid var(--qf-border)', width: '100%', overflowX: 'auto' },
  pageSizeSelect: { width: 'auto', minWidth: 70, height: 28, fontSize: 11, padding: '0 4px' },
  topPagination: { display: 'flex', alignItems: 'center', gap: 3, background: '#e8eef5', borderRadius: 999, padding: '2px 5px', border: '1px solid #c9d7e6', flexShrink: 0 },
  ...
  resetInlineBtn: { minWidth: 58, height: 24, padding: '0 8px', textTransform: 'uppercase', fontSize: 10, fontWeight: 800 },
  smartTotals: { display: 'flex', gap: 8, flexWrap: 'nowrap', alignItems: 'center', padding: '3px 10px', background: '#fff', borderTop: '1px solid var(--qf-border)', color: 'var(--qf-text-light)', fontSize: 10.5, width: '100%', overflowX: 'auto' },
  ...
  th0: { position: 'sticky', top: 0, zIndex: 10, whiteSpace: 'nowrap', fontSize: 9, padding: '6px 5px', background: 'var(--qf-navy)', color: '#fff' },
  ths: { position: 'sticky', top: 0, zIndex: 10, whiteSpace: 'nowrap', fontSize: 9, padding: '6px 5px', cursor: 'pointer', userSelect: 'none', background: 'var(--qf-navy)', color: '#fff' },
  si: { fontSize: 7, opacity: 0.45, marginLeft: 1 },
  td: { padding: '3px 5px', verticalAlign: 'middle', lineHeight: 1.15 },
