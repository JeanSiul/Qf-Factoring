...
const InvDashboardPage = () => {
  ...
  const cargar = async () => {
    ...
    } catch (e) { show('Error al cargar dashboard: ' + e.message, 'error') }
  ...
  const ultimasAsignaciones = [...asignados].sort((a, b) => new Date(b.fecha) - new Date(a.fecha)).slice(0, 5)
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16 }}>
                ...
                <tbody>
                  {ultimasAsignaciones.map(a => (
                    <tr key={a.asignacion_id}>
                      ...
                      <td style={{ fontSize: 11, color: 'var(--qf-text-light)' }}>{a.fecha ? new Date(a.fecha).toLocaleDateString('es-PE') : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {/* Requieren atención */}
        {(items.filter(i => i.estado === 'reparacion' || i.estado === 'baja').length > 0) && (
          <div className="page-card" style={{ margin: 0 }}>
                ...
                <tbody>
                  {items.filter(i => i.estado === 'reparacion' || i.estado === 'baja').map(i => (
                    <tr key={i.id}>
                      ...
                      <td><span className="badge inactive">{i.estado === 'reparacion' ? '🔧 Reparación' : '❌ Baja'}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
