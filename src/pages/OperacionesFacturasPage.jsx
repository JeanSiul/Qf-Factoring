...
const GRID_VIEW_KEY = 'qf_operaciones_facturas_grid_view_v2'
const SAVED_VIEWS_KEY = 'qf_operaciones_facturas_saved_views_v2'
const DASHBOARD_KEY = 'qf_operaciones_facturas_dashboard_v2'
...
const groupFactLabel = (row, groupBy) => {
  if (groupBy === 'status') return row.status || 'Sin estado'
  if (groupBy === 'estado_operativo') return row.estado_operativo_label || estadoOperativo(row) || 'Sin estado'
  if (groupBy === 'partner') return row.fondo_label || fondo(row) || 'Sin fondo'
  if (groupBy === 'commercial') return row.comercial_label || comercial(row) || 'Sin comercial'
  if (groupBy === 'currency') return row.currency || 'Sin moneda'
  if (groupBy === 'fecha_pago') return toDateInput(row.date_payment) || 'Sin fecha'
  return 'General'
}

const buildFactGroupSummary = (rows, groupBy) => {
  const map = new Map()
  rows.forEach(row => {
    const key = groupFactLabel(row, groupBy)
    const current = map.get(key) || { name: key, count: 0, amount: 0, net: 0, observadas: 0, pagadas: 0, registradas: 0 }
    current.count += 1
    current.amount += Number(row.amount || 0)
    current.net += Number(row.net_amount || 0)
    const general = normalize(row.status)
    const operativo = normalize(row.estado_operativo_label || estadoOperativo(row))
    if (general.includes('registr') || operativo.includes('registr')) current.registradas += 1
    if (general.includes('pag') || operativo.includes('liquid')) current.pagadas += 1
    if (general.includes('observ') || operativo.includes('observ')) current.observadas += 1
    map.set(key, current)
  })
  return [...map.values()].sort((a, b) => b.count - a.count)
}

const exportFactHtmlTable = (filename, rows) => {
  const headers = ['Cliente', 'Pagador', 'Factura', 'Moneda', 'Importe', 'Detracción', 'Comisión', 'Importe neto', 'Fecha pago', 'Fecha desembolso', 'Fondo', 'Comercial', 'Bloque', 'Estado general', 'Estado operativo']
  const htmlRows = rows.map(r => `<tr><td>${r.cliente_label || ''}</td><td>${r.pagador_label || ''}</td><td>${factura(r)}</td><td>${r.currency || ''}</td><td>${Number(r.amount || 0)}</td><td>${Number(r.detraction || 0)}</td><td>${Number(r.commission || 0)}</td><td>${Number(r.net_amount || 0)}</td><td>${formatDate(r.date_payment)}</td><td>${formatDate(r.date_payout || r.date_emission)}</td><td>${r.fondo_label || ''}</td><td>${r.comercial_label || ''}</td><td>${r.Idbloque || ''}</td><td>${r.status || ''}</td><td>${r.estado_operativo_label || ''}</td></tr>`).join('')
...
const printFactPdfReport = (rows, title = 'Reporte de Facturas QF') => {
  const byEstado = buildFactGroupSummary(rows, 'status')
  const html = `<html><head><title>${title}</title><style>body{font-family:Arial,sans-serif;color:#0f2742;padding:18px}h1{font-size:18px;margin:0 0 8px}.meta{color:#64748b;font-size:11px;margin-bottom:12px}table{width:100%;border-collapse:collapse;font-size:10px}th{background:#0f2742;color:#fff;text-align:left;padding:5px}td{border:1px solid #d9e2ec;padding:4px}.kpis{display:flex;gap:8px;margin:12px 0}.kpi{border:1px solid #d9e2ec;border-radius:8px;padding:8px;min-width:110px}.kpi b{display:block;font-size:16px;color:#185FA5}</style></head><body><h1>${title}</h1><div class="meta">Generado: ${new Date().toLocaleString('es-PE')} · Registros: ${rows.length}</div><div class="kpis"><div class="kpi"><span>Total</span><b>${rows.length}</b></div><div class="kpi"><span>Monto</span><b>${money(rows.reduce((s,r)=>s+Number(r.amount||0),0))}</b></div><div class="kpi"><span>Neto</span><b>${money(rows.reduce((s,r)=>s+Number(r.net_amount||0),0))}</b></div><div class="kpi"><span>Observadas</span><b>${rows.filter(r=>normalize(r.estado_operativo_label||estadoOperativo(r)).includes('observ')).length}</b></div></div><h2 style="font-size:14px">Resumen por estado</h2><table><thead><tr><th>Estado</th><th>Cantidad</th><th>Monto</th><th>Neto</th></tr></thead><tbody>${byEstado.map(g => `<tr><td>${g.name}</td><td>${g.count}</td><td>${money(g.amount)}</td><td>${money(g.net)}</td></tr>`).join('')}</tbody></table><h2 style="font-size:14px">Detalle</h2><table><thead><tr><th>Factura</th><th>Cliente</th><th>Pagador</th><th>Fondo</th><th>Neto</th><th>Fecha pago</th><th>Estado</th></tr></thead><tbody>${rows.map(r => `<tr><td>${factura(r)}</td><td>${r.cliente_label || ''}</td><td>${r.pagador_label || ''}</td><td>${r.fondo_label || ''}</td><td>${money(r.net_amount, r.currency)}</td><td>${formatDate(r.date_payment)}</td><td>${r.status || ''}</td></tr>`).join('')}</tbody></table><script>window.onload=()=>window.print()</script></body></html>`
...
const ModalFactura = ({ item, onClose, onSave }) => {
  ...
  const submit = async () => {
    if (!form.billing_id) return setError('Número de factura es requerido')
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 920, width: '94vw' }}>
        ...
        <div className="modal-body" style={{ maxHeight: '75vh', overflowY: 'auto' }}>
          <div style={S.g3}>
            <div className="form-group"><label className="form-label">Número factura *</label><input className="form-control" value={form.billing_id} onChange={e => set('billing_id', e.target.value)} /></div>
            <div className="form-group"><label className="form-label">Moneda</label><select className="form-control" value={form.currency} onChange={e => set('currency', e.target.value)}><option value="Soles">Soles</option><option value="Dolares">Dólares</option><option value="PEN">PEN</option><option value="USD">USD</option></select></div>
          ...
          <div style={S.g4}>
            ...
            <div className="form-group"><label className="form-label">Detracción</label><input className="form-control" type="number" step="0.01" value={form.detraction} onChange={e => set('detraction', e.target.value)} /></div>
            <div className="form-group"><label className="form-label">Comisión</label><input className="form-control" type="number" step="0.01" value={form.commission} onChange={e => set('commission', e.target.value)} /></div>
          ...
          <div style={S.g4}>
            ...
            <div className="form-group"><label className="form-label">Fecha emisión</label><input className="form-control" type="date" value={form.date_emission} onChange={e => set('date_emission', e.target.value)} /></div>
            <div className="form-group"><label className="form-label">Días</label><input className="form-control" value={calcDays(form)} disabled /></div>
          </div>
          <div style={S.g4}>
...
const OperacionesFacturasPage = () => {
  ...
  const quickFilteredData = useMemo(() => {
    ...
    return facturas.filter(row => {
      ...
      if (quickPreset === 'registered') return general.includes('registr') || operativo.includes('registr')
      if (quickPreset === 'paid') return general.includes('pag') || operativo.includes('liquid')
      if (quickPreset === 'observed') return general.includes('observ') || operativo.includes('observ')
      if (quickPreset === 'confirmed') return general.includes('confirm') || operativo.includes('valid')
      if (quickPreset === 'soles') return normalize(row.currency).includes('sol') || normalize(row.currency).includes('pen')
      if (quickPreset === 'dollars') return normalize(row.currency).includes('dol') || normalize(row.currency).includes('usd')
      if (quickPreset === 'gerencia') return general.includes('observ') || operativo.includes('observ') || Number(row.net_amount || 0) > 0
      if (quickPreset === 'operaciones') return general.includes('registr') || general.includes('proceso') || operativo.includes('valid')
  ...
  const agRows = useMemo(() => quickFilteredData.map(row => ({
    ...row,
    cliente_label: cliente(row),
    pagador_label: pagador(row),
    factura_label: factura(row),
    fondo_label: fondo(row),
    comercial_label: comercial(row),
    estado_operativo_label: estadoOperativo(row),
  })), [quickFilteredData])
  ...
  const refreshPaginationInfo = api => {
    if (!api) return
    const totalGridPages = Math.max(api.paginationGetTotalPages?.() || 1, 1)
    const current = Math.min((api.paginationGetCurrentPage?.() || 0) + 1, totalGridPages)
    setGridPageInfo({ current, total: totalGridPages })
  }
  ...
  const limpiarFiltrosTabla = () => {
    ...
    gridApi.applyColumnState({
      ...
      state: [{ colId: 'billing_id', sort: 'desc' }],
    })
  ...
  const resetGridView = () => {
    ...
    gridApi.setColumnsVisible(['date_emission', 'detraction', 'commission', 'Other_dsctos', 'observaciones'], false)
    gridApi.setGridOption?.('quickFilterText', '')
  ...
  const exportCsv = () => {
    ...
    const headers = ['Cliente', 'Pagador', 'Factura', 'Moneda', 'Importe', 'Detracción', 'Comisión', 'Otros descuentos', 'Importe neto', 'Fecha pago', 'Fecha desembolso', 'Fondo', 'Comercial', 'Bloque', 'Estado general', 'Estado operativo']
    const body = rows.map(r => [
      ...
      r.Idbloque,
      r.status,
      r.estado_operativo_label,
    ].map(csvEscape).join(';'))
    downloadTextFile(`facturas_${new Date().toISOString().slice(0, 10)}.csv`, [headers.join(';'), ...body].join('\n'))
  }

  const exportExcel = () => exportFactHtmlTable(`facturas_${new Date().toISOString().slice(0, 10)}.xls`, getDisplayedRows())
  const exportPdf = () => printFactPdfReport(getDisplayedRows(), 'Reporte de Facturas QF')

  const applyColumnPreset = preset => {
    if (!gridApi) return
    const allCols = ['cliente_label', 'pagador_label', 'billing_id', 'currency', 'amount', 'detraction', 'commission', 'Other_dsctos', 'net_amount', 'date_payment', 'date_payout', 'date_emission', 'fondo_label', 'comercial_label', 'Idbloque', 'status', 'estado_operativo_label', 'observaciones', 'acciones']
    const presets = {
      gerencia: ['cliente_label', 'pagador_label', 'billing_id', 'net_amount', 'date_payment', 'fondo_label', 'status', 'acciones'],
      operaciones: ['cliente_label', 'pagador_label', 'billing_id', 'currency', 'net_amount', 'date_payment', 'date_payout', 'fondo_label', 'estado_operativo_label', 'acciones'],
      auditoria: ['billing_id', 'cliente_label', 'amount', 'detraction', 'commission', 'Other_dsctos', 'net_amount', 'date_payment', 'fondo_label', 'Idbloque', 'status', 'acciones'],
      completo: allCols,
    }
  ...
  const agColumnDefs = useMemo(() => [
    { headerName: 'Cliente', field: 'cliente_label', width: 180, filter: 'agTextColumnFilter', cellStyle: { fontWeight: 700, color: 'var(--qf-navy)' } },
    { headerName: 'Pagador', field: 'pagador_label', width: 190, filter: 'agTextColumnFilter' },
    { headerName: 'Factura', field: 'billing_id', width: 105, sort: 'desc', cellRenderer: p => <code style={S.opCode}>{p.value || factura(p.data) || '-'}</code>, filter: 'agTextColumnFilter' },
    { headerName: 'Moneda', field: 'currency', width: 80, filter: 'agTextColumnFilter', headerClass: 'qf-center-header' },
    { headerName: 'Importe', field: 'amount', width: 120, type: 'numericColumn', cellStyle: { fontWeight: 800, color: '#e65100', textAlign: 'center' }, cellClass: 'qf-right-cell', valueFormatter: p => money(p.value, p.data?.currency), filter: 'agNumberColumnFilter' },
    { headerName: 'Detracción', field: 'detraction', width: 115, type: 'numericColumn', hide: true, cellStyle: { fontWeight: 800, color: '#c62828', textAlign: 'center' }, cellClass: 'qf-right-cell', valueFormatter: p => money(p.value, p.data?.currency), filter: 'agNumberColumnFilter' },
    { headerName: 'Comisión', field: 'commission', width: 110, type: 'numericColumn', hide: true, cellStyle: { fontWeight: 800, color: '#c62828', textAlign: 'center' }, cellClass: 'qf-right-cell', valueFormatter: p => money(p.value, p.data?.currency), filter: 'agNumberColumnFilter' },
    { headerName: 'Otros Dsctos.', field: 'Other_dsctos', width: 120, type: 'numericColumn', hide: true, cellStyle: { fontWeight: 800, color: '#c62828', textAlign: 'center' }, cellClass: 'qf-right-cell', valueFormatter: p => money(p.value, p.data?.currency), filter: 'agNumberColumnFilter' },
    { headerName: 'Importe Neto', field: 'net_amount', width: 130, type: 'numericColumn', cellStyle: { fontWeight: 800, color: '#2e7d32', textAlign: 'center' }, cellClass: 'qf-right-cell', valueFormatter: p => money(p.value, p.data?.currency), filter: 'agNumberColumnFilter' },
    { headerName: 'Fecha Pago', field: 'date_payment', width: 125, valueFormatter: p => formatDate(p.value), filter: 'agDateColumnFilter', headerClass: 'qf-center-header' },
    { headerName: 'Fecha Desembolso', field: 'date_payout', width: 145, valueGetter: p => p.data?.date_payout || p.data?.date_emission, valueFormatter: p => formatDate(p.value), filter: 'agDateColumnFilter', headerClass: 'qf-center-header' },
    ...
    { headerName: 'Fondo', field: 'fondo_label', width: 140, cellRenderer: p => <span style={S.bankPill}>{p.value || '-'}</span>, filter: 'agTextColumnFilter' },
    { headerName: 'Comercial', field: 'comercial_label', width: 135, filter: 'agTextColumnFilter' },
    { headerName: 'Bloque', field: 'Idbloque', width: 90, filter: 'agNumberColumnFilter', headerClass: 'qf-center-header' },
    { headerName: 'Estado General', field: 'status', width: 135, cellRenderer: p => <span className={`badge ${badgeClass(p.value)}`} style={{ fontSize: 8 }}>{String(p.value || '-').toUpperCase()}</span>, filter: 'agTextColumnFilter' },
    { headerName: 'Estado Operativo', field: 'estado_operativo_label', width: 145, cellRenderer: p => <span className={`badge ${badgeClass(p.value)}`} style={{ fontSize: 8 }}>{String(p.value || '-').toUpperCase()}</span>, filter: 'agTextColumnFilter' },
    { headerName: 'Observaciones', field: 'observaciones', flex: 1, minWidth: 160, hide: true, filter: 'agTextColumnFilter' },
    {
      headerName: 'Acciones',
      field: 'acciones',
      width: 115,
  ...
  const groupSummary = useMemo(() => buildFactGroupSummary(liveRows, groupBy), [liveRows, groupBy])
  const estadoSummary = useMemo(() => buildFactGroupSummary(liveRows, 'status'), [liveRows])
  const fondoSummary = useMemo(() => buildFactGroupSummary(liveRows, 'partner'), [liveRows])
  const comercialSummary = useMemo(() => buildFactGroupSummary(liveRows, 'commercial'), [liveRows])

  return (
    <div className="fade-in" style={S.page}>
      <ToastContainer toasts={toasts} />
      <style>{`
        .qf-tareas-grid .ag-root-wrapper { border: 0; border-top: 1px solid var(--qf-border); font-family: Montserrat, Arial, sans-serif; }
        .qf-tareas-grid .ag-header { background: var(--qf-navy); color: #fff; border-bottom: 0; }
        .qf-tareas-grid .ag-header-cell, .qf-tareas-grid .ag-header-group-cell { background: var(--qf-navy); color: #fff; font-weight: 800; text-transform: uppercase; letter-spacing: .2px; border-right: 0; }
        .qf-tareas-grid .ag-header-cell-text { color: #fff; font-size: 8.5px; }
        .qf-tareas-grid .ag-header-cell { padding-left: 2px; padding-right: 2px; line-height: 1; }
        .qf-tareas-grid .ag-icon, .qf-tareas-grid .ag-header-icon { color: #fff; font-size: 15px; }
        .qf-tareas-grid .ag-floating-filter { background: #f8fafc; border-bottom: 1px solid var(--qf-border); min-height: 10px; }
        .qf-tareas-grid .ag-floating-filter-body { width: 100%; }
        .qf-tareas-grid .ag-floating-filter-input, .qf-tareas-grid .ag-input-field-input {
          min-height: 2px; height: 6px; padding: 0 2px 0 17px !important; font-size: 8px; border-radius: 7px;
          border: 1px solid #9fb2c8 !important; background-color: #ffffff !important;
          ...
          background-repeat: no-repeat !important; background-position: 4px calc(100% - 3px) !important; background-size: 10px 10px !important;
          color: var(--qf-navy); box-shadow: inset 0 0 0 1px rgba(24,95,165,.08);
        }
        .qf-tareas-grid .ag-floating-filter-button { margin-left: 6px; }
        .qf-tareas-grid .ag-floating-filter-button-button { min-width: 22px; height: 22px; width: 22px; border-radius: 7px; border: 1px solid #9fb2c8; background: linear-gradient(135deg, #ffffff, #e8eef5); box-shadow: inset 0 0 0 1px rgba(24,95,165,.08); }
        .qf-tareas-grid .ag-floating-filter .ag-icon-search, .qf-tareas-grid .ag-floating-filter .ag-icon-filter, .qf-tareas-grid .ag-floating-filter .ag-icon { color: #185FA5 !important; font-size: 13px !important; }
        .qf-tareas-grid .ag-row { border-bottom: 1px solid var(--qf-border); }
        .qf-tareas-grid .ag-row-hover { background: #f8fafc; }
        .qf-tareas-grid .ag-paging-panel { min-height: 24px; font-size: 8px; color: var(--qf-text-light); border-top: 1px solid var(--qf-border); }
        .qf-tareas-grid .ag-cell { display: flex; align-items: center; padding-top: 0 !important; padding-bottom: 0 !important; line-height: 1 !important; }
        .qf-tareas-grid .qf-right-cell { justify-content: flex-end !important; text-align: right !important; padding-right: 4px !important; }
        .qf-tareas-grid .qf-center-header .ag-header-cell-label { justify-content: center; }
        .qf-tareas-grid .qf-right-header .ag-header-cell-label { justify-content: flex-end; }
        .qf-tareas-grid .ag-header-cell[col-id="date_payment"] .ag-input-field-input,
        .qf-tareas-grid .ag-header-cell[col-id="date_payout"] .ag-input-field-input,
        .qf-tareas-grid .ag-header-cell[col-id="date_emission"] .ag-input-field-input {
          background-image: none !important; background: #ffffff !important; padding-left: 4px !important;
        }
      `}</style>
        ...
        <div style={S.stickyTools}>
          <div style={S.pagRow}>
            ...
            <select className="filter-input" value={quickPreset} onChange={e => setQuickPreset(e.target.value)} style={S.erpSelect}>
              ...
              <option value="confirmed">Confirmadas / Validadas</option>
            ...
            <button className="btn btn-secondary btn-sm" onClick={limpiar}>Limpiar búsqueda</button>
          ...
          {showColumnPanel && (
            <div style={S.columnPanel}>
              {[
                ...
                ['net_amount', 'Neto'],
                ['date_payment', 'F. pago'],
                ['date_payout', 'F. desemb.'],
                ['date_emission', 'F. emisión'],
                ['fondo_label', 'Fondo'],
                ['comercial_label', 'Comercial'],
                ['Idbloque', 'Bloque'],
                ['status', 'Estado'],
                ['estado_operativo_label', 'Est. op.'],
                ['observaciones', 'Obs.'],
                ['acciones', 'Acciones'],
              ].map(([field, label]) => (
          ...
          <div style={S.smartTotals}>
            ...
            <span><b>{liveRows.filter(r => normalize(r.estado_operativo_label).includes('observ')).length}</b> observadas</span>
          ...
          {loading && facturas.length === 0 ? (
            ...
            <AgGridReact
              ...
              onGridReady={params => {
                ...
                params.api.setColumnsVisible(['date_emission', 'detraction', 'commission', 'Other_dsctos', 'observaciones'], false)
...
const S = {
  ...
  erpSelectSmall: { minWidth: 150, height: 24, fontSize: 9.5, padding: '0 20px 0 7px' },
