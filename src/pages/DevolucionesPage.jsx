...
const ControlInversionistas = () => {
  ...
  const openEdit = (row) => {
    ...
    setModal({
      ...
      form: {
        ...
        naturaleza: row.naturaleza === 'PJ' ? 'PJ' : 'PN',
  ...
  const validateDocument = async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({ tipo_documento: tipo_documento === 'RUC' ? 'RUC' : 'DNI', nro_documento: numero_documento })
      ...
      setModal(prev => ({
        ...
        form: {
          ...prev.form,
          razon_social: payload.razon_social || payload.nombre || prev.form.razon_social,
          nombre: payload.nombre || prev.form.nombre,
          apellido: payload.apellido || `${payload.apellido_paterno || ''} ${payload.apellido_materno || ''}`.trim() || prev.form.apellido,
  ...
  const handleFormChange = (name, value) => {
    ...
    if (name === 'tipo_documento' && modal.mode === 'create') {
      ...
      apiCall(`${API_BASE}/siguiente-codigo?naturaleza=${naturaleza}`).then(res => {
        ...
        <div style={styles.formGrid4}>
          <Field label="Banco">
            <select className="form-control" value={form.banco || ''} onChange={e => onChange('banco', e.target.value)} style={{ width: '100%', minWidth: '180px' }}>
              ...
              {activeBanks.map(b => { const bankId = getField(b, 'id', 'ID'); const bankName = getField(b, 'name', 'nombre', 'Name'); return <option key={bankId} value={bankId} title={bankName}>{bankName}</option> })}
            </select>
          </Field>
          <Field label="Moneda">
            <select className="form-control" value={form.moneda || ''} onChange={e => onChange('moneda', e.target.value)} style={{ width: '100%', minWidth: '120px' }}>
              <option value="">Seleccione...</option>
...
const styles = {
  ...
  actionBar: { display: 'flex', justifyContent: 'flex-end', marginBottom: 8 },
  ...
  th0: { position: 'sticky', top: 0, zIndex: 10, whiteSpace: 'nowrap', fontSize: 9, padding: '5px 4px', background: '#f0f7ff', borderBottom: '1px solid #d4e0e9' },
  ths: { position: 'sticky', top: 0, zIndex: 10, whiteSpace: 'nowrap', fontSize: 9, padding: '5px 4px', cursor: 'pointer', userSelect: 'none', background: '#f0f7ff', borderBottom: '1px solid #d4e0e9' },
  si: { fontSize: 7, opacity: 0.45, marginLeft: 1 },
  td: { padding: '3px 4px', verticalAlign: 'middle', lineHeight: 1.15, borderBottom: '1px solid #eef2f6' },
