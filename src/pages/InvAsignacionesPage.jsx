...
const ModalAsignar = ({ item, usuarios, onClose, onSave, isReasignacion = false }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSave({ ...form, item_id: item.item_id || item.id, accion: isReasignacion ? 'reasignacion' : 'asignacion' })
      onClose()
    } catch (e) {
      ...
      <div className="modal" style={{ maxWidth: 440 }}>
        ...
        <div className="modal-body">
          <div style={{ background: '#e8eef5', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 13 }}>
            ...
            {isReasignacion && item.usuario_nombre && (
              <div style={{ fontSize: 11, color: 'var(--qf-text-light)', marginTop: 4 }}>Actualmente asignado a: <strong>{item.usuario_nombre}</strong></div>
            )}
          </div>
...
const ModalDevolver = ({ item, onClose, onSave }) => {
  ...
  const handleSubmit = async () => {
    setLoading(true)
    try {
      await onSave({ item_id: item.item_id, usuario_id: item.usuario_id, usuario_nombre: item.usuario_nombre, observacion })
      onClose()
    } catch (e) {
      console.error(e)
    } finally { setLoading(false) }
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 400 }}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group">
            <label className="form-label">Observación</label>
            <textarea className="form-control" value={observacion} onChange={e => setObservacion(e.target.value)} rows={2} style={{ resize: 'vertical' }} placeholder="Motivo de devolución, estado del equipo..." />
          </div>
        </div>
        <div className="modal-footer">
          <button className="btn btn-secondary" onClick={onClose}>Cancelar</button>
        ...
        <div className="modal-body" style={{ maxHeight: '60vh', overflowY: 'auto' }}>
          {loading ? (
            ...
            <table className="qf-table">
              <thead>
                <tr><th>Acción</th><th>Usuario</th><th>Realizado por</th><th>Fecha</th><th>Observación</th></tr>
              </thead>
              <tbody>
                {historial.map(h => {
...
const InvAsignacionesPage = () => {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      setUsuarios(Array.isArray(us) ? us.filter(u => u.estado === 1) : [])
    } catch (e) {
  ...
  const handleAsignar = async (form) => {
    const res = await apiCall('/qf/inv/asignaciones/asignar', {
      ...
      body: JSON.stringify({
        ...form,
        usr_realizo_id: user?.id,
        usr_realizo: user?.nombres || user?.username,
      })
    })
    ...
    <div className="fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 12, marginBottom: 20 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: '14px 18px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', borderTop: '3px solid #4caf50' }}>
          ...
          <div style={{ fontSize: 28, fontWeight: 800, color: '#2e7d32', fontFamily: 'Montserrat', lineHeight: 1.2 }}>
            {new Set(asignados.map(a => a.usuario_id)).size}
          </div>
        </div>
      </div>
      ...
      <div className="page-card">
        ...
        <div style={{ overflowX: 'auto' }}>
          {loading ? (
            ...
            <div className="empty-state"><div className="icon">👤</div><p>No hay items asignados actualmente</p></div>
          ) : (
            <table className="qf-table">
              <thead>
                <tr>
                  ...
                  <tr key={a.asignacion_id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 4, justifyContent: 'center', flexWrap: 'wrap' }}>
                        <button className="btn btn-secondary btn-sm" onClick={() => setModal({ type: 'historial', data: a })} title="Ver historial">📋</button>
                        <button className="btn btn-primary btn-sm" onClick={() => setModal({ type: 'reasignar', data: a })} title="Reasignar">🔄</button>
                        <button className="btn btn-warning btn-sm" onClick={() => setModal({ type: 'devolver', data: a })} title="Registrar devolución">↩️</button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        {!loading && (
          <div style={{ padding: '12px 24px', borderTop: '1px solid var(--qf-border)', fontSize: 12, color: 'var(--qf-text-light)' }}>
            {filtrados.length} de {asignados.length} asignaciones
          </div>
        )}
      </div>
